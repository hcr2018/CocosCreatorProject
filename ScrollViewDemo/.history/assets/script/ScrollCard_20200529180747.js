...
cc.Class({
    ...
    touchEnd(event) {
        ...
        self.onclckItem(node,yxStyle[node.name]);
        this.yxStyles = yxStyle[node.name];

       // this.data = this.yxStyles;
       this.data ='简单模式';

        
    },

    onclckItem:function(node,yxStyle){
        console.log('获取当前节点'+node.name,'执行响应事件'+yxStyle );
    },

    onclickPlayButton:function(){

        console.log('执行响应事件'+this.yxStyles );
        //切换场景 
        cc.director.loadScene('loading_ct');
    },
